...
public class BoardDAO_seq {
	...
	public List<BoardBean> getBoardList(int page, int limit) {
		...
		String board_list_sql = "select * from " + "(select rownum rnum,board_num,Board_name,"
				...
				+ "board_readcount,board_date from" + "(select * from board11 " + " order by board_re_ref desc,"
	...
	public boolean boardInsert(BoardBean board) {
		...
		try {
			...
			sql = "insert into board11 " + "(BOARD_NUM,BOARD_NAME,BOARD_PASS,BOARD_SUBJECT,"
					+ " BOARD_CONTENT, BOARD_FILE, BOARD_RE_REF," + " BOARD_RE_LEV, BOARD_RE_SEQ,BOARD_READCOUNT,"
					+ " BOARD_DATE) " + " values(board11_seq.nextval,?,?,?,?,?,board11_seq.nextval,?,?,?,sysdate)";
			...
			pstmt.setInt(6, 0); // BOARD_RE_LEV 필드 //원본글 댓글이달릴때마다 레벨이올라간다.
	...
	public boolean boardModify(BoardBean boarddata) {
		String sql = "update board11 set BOARD_SUBJECT = ?, BOARD_CONTENT = ? where BOARD_NUM = ? "; 
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(sql);
	...
	public boolean boardDelete(int num) {
		...
		String sql = "delete board11 where BOARD_NUM = ? ";
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
